...
int process1(char s){				//ham bien doi ki tu ve dang int
	return (int(s)<60? int(s)-48 : int(s)-55);			//tra ve ki tu dang int dua theo bang ASCII 
}
...
checksum(int count, char s[100],int err){
	int sum = 0;									//tao bien sum = 0;
	char s1,s2;										//khai bao bien s1s2 luu 2 gia tri lien tiep trong s
	for(int i=2;i<100;i=i+2){						//cho bien i chay tu 0 den 100 voi step = 2
...
bytecount(int count,char s[100],int err){
	...
	if(dem!=sum){												//neu so ki tu dem duoc khac byte count
		...
		cout<<"loi byte count o dong : "<<count<<endl;			//hien thi dong cho loi byte count dong
...
checkword(int count,char s[100],int err){			//kiem tra loi ki tu trong string s va tra ve gia tri error
	int kiemTra =0;									//ktra luu so ki tu khong hop le
	for(int i=1;i<100;i++){							//ktra tu ki tu thu 2 .  Neu ma ki tu khac voi ma hexa
	...
	if(kiemTra > 0){									//neu xuat hien ki tu khong thuoc hexa
		err = err+1 ;									//err = err +1;
		cout<<"ki tu k hop le dong "<<count<<endl;		//hien thi dong ki tu khong hop le
...
checkadd(int count,char s[100],data *t,int countAddS5){
	int l=0;							//l la bien luu do dai ki tu khac NULL trong string s ta truyen
	for(int i=0;i<100;i++){				//cho i chay tu 0 den 100
	...
	for(int i=0;i<l-4;i++){							//cho i chay tu 0 den ki tu xuong dong -4.tru 4 tru 2 ki tu dau va 2 ki tu checksum o cuoi string
	...
	return countAddS5;				//tra ve so ki tu address dong truoc dong  S5
...
//		cout<<"err = "<<err<<endl;							//hien thi error
		...
		cout<<" writing..."<<endl;							//hien thi dong chu writing
		fstream f;											//khai bao bien f de lam viec voi file
		f.open("write_srecord.txt",ios ::app);				//cho phep viet them vao cuoi file write_freg.txt.neu file chua tao thi he thong se tao 1 file 
//		f.open("write_freg.txt",ios ::app);					//cho phep viet them vao cuoi file write_freg.txt.neu file chua tao thi he thong se tao 1 file
		f<<t->add <<" <------- "<<t->data;					//ghi temp vao f dang address <---- data
		f<<endl;											//ghi ki tu xuong dong trong file moi
		f.close();							 				//dong file
	}
...
process(char s[100],int count,int err,int countAddS5){		//xu lu dong str sau khi lay ra tu file va countAddS5
	...
	if(s1=='S' ){			//neu ki tu dau tien la S
		if(s[1]<'0' || s[1]>'9'){		//neu ki tu thu 2 khac 0 hoac 9
			cout<<" ki tu sau S k hop le dong "<<count<<endl;	//hien thi "ki tu sau S k hop le'
		...
		cout<<"loi ki tu S dau tien "<<endl;					//Hien thi loi ki tu S dau tien
		cout<<"loi o dong "<<count<<" :"<<endl;					//in ra loi o dong 
			for(int i=0;i<100;i++){								//cho i chay tu 0 -> 100
				cout<<s[i];										//hien tung phan tu cua string s ma ta dua vao
			}
		cout<<endl;												//hien thi ki tu xuong dong
	}
...
int main(int argc, char** argv) {
	FILE *fp;								//con tro fp dung trong FILE
...
//	fp = fopen("freg","r");
	...
	if(fp==NULL){								//neu file trong
		cout<<"can't read file"<<endl;			//hien thi khong doc duoc file
	}
	...
	cout<<"\ndone"<<endl;						//hien thi ki tu done khi ket thuc chuong trinh
